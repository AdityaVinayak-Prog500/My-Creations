...
def load_current_cpi_components():
    """
    Load current 2024=100 rural and urban CPI general indices
    for dashboard display only.

    The component series are kept separate from the econometric
    model datasets so the CPI base-year revision does not alter
    ...
    for filename, label in [
        ...
        if "series" in df.columns:
            current_mask = (
                df["series"].astype(str).str.strip().str.lower()
                == "current"
            )
            if current_mask.any():
                df = df.loc[current_mask].copy()
        ...
        if "state" in df.columns:
            ...
            if india_mask.any():
                df = df.loc[india_mask].copy()

        # Some downloaded files label the component in `sector`.
        if "sector" in df.columns:
            component_mask = (
                df["sector"].astype(str).str.contains(
                    label.replace("cpi_", ""),
                    case=False,
                    na=False,
                )
            ...
            if component_mask.any():
                df = df.loc[component_mask].copy()

        value_col = None
        for candidate in ["index", "value"]:
    ...
    if not frames:
        return pd.DataFrame(columns=["date", "cpi_rural", "cpi_urban"])
...
# ============================================================
    ...
    for col, label in [
        ...
        if col in transmission.columns:
            ...
            temp = temp.rename(columns={col: "Inflation"})
            upstream.append(temp)
    ...
    if upstream:
        ...
        st.caption(
            "WPI and Output PPI use the latest available observations "
            "from the project's dedicated transmission dataset."
        )
